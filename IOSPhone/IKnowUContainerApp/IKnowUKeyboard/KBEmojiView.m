...
- (void)handlePress:(id)sender
{
    ...
    [self.data.s setString:dataString];
    self.data.tag = button.tag;
    
    if ([button isKindOfClass:[KBBackSpaceButton class]])
    {
        self.data.isBackSpace = YES;
    }
    
    if ([sender isKindOfClass:[KBKeyButton class]])
    {
        self.data.isEmoji = true;
    }
    
    if ([button isKindOfClass:[KBNextInputModeButton class]])
    {
        [self.data.s setString:@"nx"];
    }
