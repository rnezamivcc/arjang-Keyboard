 ...
 *
 */
public class PredictionSuggestion extends Suggestion {
    ...
    public PredictionSuggestion(Context context) {
        super(context);
    ...
    private void setLatinText(String rootText, String suggestion) {
        ...
        {
            case CAPS_ALL:
                this.textShown = textShown.toUpperCase();
                break;
            case CAPS_FIRST_LETTER:
                char[] chrs = this.textShown.toCharArray();
                chrs[0] = Character.toUpperCase(chrs[0]);
                this.textShown = new String(chrs);
                break;
        }
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (!this.longPressOccurred && this.sawDownEvent) {
                        ...
                        if (this.isRoot) {
                            //final byte[] prefs = new byte[] {1};
                            this.service.kbContainerView.getKbLinearLayout().getKeyboardView().onReleaseKey(this.service.kbContainerView.getKbLinearLayout().getKeyboardView().getKeyboard().getKeyFromPrimaryCode(32));
                            //this.service.handleWordSeparator(32);
                        } else {
                            this.service.pickSuggestionManually(this.rawPrediction, true, this.capsMode);
                        }
                    }
